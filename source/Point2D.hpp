...
public:
    ...
    void draw(FrameBuffer& buffer, ZBuffer& zb, const std::vector<LightSource*>& lights, const std::vector<glm::dvec3>& lights_pos, const Material& m) {
        if (zb.fill_at(x, y, z)) {
            glm::dvec3 c(0.);
            for (uint32_t i = 0; i < lights.size(); ++i) {
                ...
                case LightSourceType::Ambient: {
                    c += m.get_Kd() * m.get_map_Kd_at({u, v}) * l->colorAt(l_pos, coord);
                    break;
                }
                ...
                default: {
                    ...
                    glm::dvec3 t = l->colorAt(l_pos, coord) * 
                                    (m.get_Kd() * m.get_map_Kd_at({u, v}) * std::max(glm::dot(norm, l_dir), 0.) + 
                                    m.get_Ks() * (double)(glm::dot(l_dir, norm) > 0) * std::pow(std::max(glm::dot(h, norm), 0.), m.get_Ns()));
                    c += t;
                    break;
                }
                }
